...
export default function Page() {
      ...
      <div className='h-screen w-full bg-white-100'>
          ...
          {[9, 29, 59].map((time) => (
            <button
              ...
              key={time}
              className={`py-2 px-4 rounded-lg ${selectedTime === time ? 'bg-blue-500 text-white' : 'bg-white-500 text-gray-700 border'}`}
            >
              {time === 9 && '< 10 Minutes'}
              {time === 29 && '< 30 Minutes'}
              {time === 59 && '< 1 hour'}
  ...
  {recipeArray.length > 0 ? recipeArray.map((recipe) => (
    ...
    <p className='text-center col-span-2'>Your Pantry is Empty. Select Some Ingredients to Begin.</p>
  )}
</div>
          ...
          <Link legacyBehavior href="/login"><a className='text-green-500'>Log in</a></Link>
        </div>
      </div>
